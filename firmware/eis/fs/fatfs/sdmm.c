...
#include "eis.h"		/* Include device specific declareation file here */

#define DO_INIT()						/* Initialize port for MMC DO as input */
#define DO			gpio_get(EIS_SD_MISO)	/* Test for MMC DO ('H':true, 'L':false) */

#define DI_INIT()						/* Initialize port for MMC DI as output */
#define DI_H()		gpio_put(EIS_SD_MOSI, 1)	/* Set MMC DI "high" */
#define DI_L()		gpio_put(EIS_SD_MOSI, 0)	/* Set MMC DI "low" */

#define CK_INIT()						/* Initialize port for MMC SCLK as output */
#define CK_H()		gpio_put(EIS_SD_SCK, 1)	/* Set MMC SCLK "high" */
#define	CK_L()	gpio_put(EIS_SD_SCK, 0)	/* Set MMC SCLK "low" */

#define CS_INIT()							/* Initialize port for MMC CS as output */
#define	CS_H()	gpio_put(EIS_SD_SS, 1)	/* Set MMC CS "high" */
#define CS_L()		gpio_put(EIS_SD_SS, 0)	/* Set MMC CS "low" */
...
)
{
	...
	do {
		...
		CK_H(); dly_xx(DLY); CK_L(); dly_xx(DLY);
